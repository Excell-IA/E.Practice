...
import type { DemoNote } from "@/lib/demo-state";

type NotesGroupNodeProps = {
  notes: DemoNote[];
  startDate: string;
  endDate: string;
  x: number;
...
export function NotesGroupNode({ notes, startDate, endDate, timelineY, x, y, onSelect }: NotesGroupNodeProps) {
  ...
  const titleLabel = isMulti ? `${count} note` : `Nota di ${single.author.name}`;
  const hoverTopLabel = isMulti ? `${count} note` : "NOTA";
  const hoverBottomLabel = isMulti ? "Clicca per la lista" : single.author.name;
  ...
  return (
    ...
    >
      ...
      <circle className="fill-transparent" cy={timelineY} r="22" />
      <circle className="fill-electric/10 stroke-electric/20 stroke-2 transition-colors group-hover:fill-electric/20 group-hover:stroke-electric/50" cy={timelineY} r="16" />
      ...
      <g transform={`translate(0 ${y})`}>
        ...
        <circle className="fill-electric/10 stroke-electric stroke-[1.6]" r="20" />
        {isMulti ? (
